...
2. **Write the cost row** for each subagent from the harness completion notification (a subagent
   ...
   `node ${TPM_HOME}/tools/workflow/cost-ledger.js --epic-path dev/<epic> --agent <id> --role <role>
...
3. **Cost rollup** — `node ${TPM_HOME}/tools/workflow/cost-ledger.js --rollup dev/<epic> --summary` for the
   epic-level cost provenance reviewed at close (what the epic cost, per subagent).
4. **Structure audit** — `node ${TPM_HOME}/tools/workflow/audit.js --out <tmp>/audit.md` to confirm numbering
