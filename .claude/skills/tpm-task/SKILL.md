...
selector, confirm destructive ops), then forwards the MECHANICS to `npx tpm task` and relays
...
- **Semantic** (`the auth ones`, `anything about the rename`): read candidate headlines via
  `npx tpm task list`/`show`, pick the matching ids yourself, **echo the resolved concrete id set for
...
- **`list`** → `npx tpm task --tasks-dir <dir> list [--state <s>]`. Default pool is open + in-progress;
...
- **`show`** → resolve the selector, then `npx tpm task show <selector> [--state <s>]`. For a semantic
  ask, echo the resolved ids first.
- **`add-subtask`** → `npx tpm task add-subtask <id> "<text>"`. Appends ONE lettered subtask, PRESERVING
...
- **`check`** → `npx tpm task check <id.LETTER>` (e.g. `1245.A`; `1245a`/`#1245.A` also accepted). Ticks
...
- **`start`** → `npx tpm task start <id>` (→ in-progress). Redundant/illegal transitions are the tool's
