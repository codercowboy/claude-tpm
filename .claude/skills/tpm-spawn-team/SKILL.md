...
rosters + their order are authoritative from `config-resolver.js` `getDefaults().teams`; do not reorder or
re-roster them.
...
+ models resolve from `config-resolver.js`; each subagent writes ONLY in its own folder.
