...
1. **`scaffold-subagent.js`** `add-phase` / `add-round` — creates the phase work folder: per-role
...
2. **`compose-spawn-prompt.js`** — emit the standard prompt: env step-zero, role-conditional read-order +
...
4. **`lint-subagent-prompt.js`** `--file <prompt> [--verifier|--test-writer|--documentarian|--bug-fixer]` — the
