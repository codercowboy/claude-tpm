...
> **Your reading chain is [`reading-list.md`](./reading-list.md)** — the single source of truth for what a subagent reads, including the conditional blocks for resume / shipping / live-system / verifier spawns. It is complete; you need nothing outside it. the subagent-prompt lint (`npx tpm workflow lint`) derives its checks from that same file, so the documented chain and the enforced chain cannot drift.
