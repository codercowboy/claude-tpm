#!/usr/bin/env node
/**
 * doctor.js — a fail-loud PREFLIGHT for the workflow install seams. Every check here corresponds to a
 ...
 *
 * USAGE:  node tools/workflow/doctor.js [--project-root <dir>] [--json]
...
function checkCharters(projectRoot) {
  let resolved;
  try { resolved = require('./config-resolver.js').resolveConfig(undefined, { startDir: projectRoot }).resolved; }
  ...
  if (missing.length) {
    return {
      ...
      fix: 'default charters live under the BUNDLE — config-resolver.charterHome() self-locates it; a bare/relative charterFile resolves at the project root instead. Run `config-resolver.js --validate`.',
    };
  }
...
function checkGateHook(settings) {
  return hookWired(settings, 'Agent', 'gate-spawn.js')
    ...
    : { ok: false, detail: 'no spawn-gate hook wired for the Agent tool', fix: 'add a PreToolUse `Agent|Task` hook → tools/workflow/hooks/gate-spawn.js in .claude/settings.json (the scaffolder does this). Without it, Gate B is a no-op.' };
}
...
function checkCompose() {
  ...
  try {
    ...
    const out = cp.execFileSync('node', [
      path.join(BUNDLE, 'tools', 'workflow', 'compose-spawn-prompt.js'),
...
function main(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    if (argv[i] === '--project-root') projectRoot = argv[++i];
      ...
      process.stdout.write('usage: doctor.js [--project-root <dir>] [--json]\n'); process.exit(0);
    }
  }
