#!/usr/bin/env node
/**
 * audit.js (v2 — epic-aware) — walk `dev/` and audit each top-level entry
 ...
 *
 * Usage:
 *   node tools/audit.js --out <path-to-output.md> [--tasks-root <dir>] [--strict]
 *
 * Flags:
...
for (let i = 0; i < argv.length; i++) {
  if (argv[i] === '--out') OUT_PATH = argv[++i];
    ...
    console.log('usage: audit.js --out <path-to-output.md> [--tasks-root <dir>] [--strict]');
