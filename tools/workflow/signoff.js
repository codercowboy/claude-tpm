#!/usr/bin/env node
/**
 * signoff.js — the two-token user-sign-off ledger for tpm-workflow kickoffs.
 *
 * WHY
 ...
 *   PreToolUse spawn-gate hook (hooks/gate-spawn.js) can verify — code, not judgment.
 *
 ...
 *
 * USAGE
 *   node signoff.js questions --roster "<one-line>"                    # token 1 (after the user answers)
 *   node signoff.js spawn --round "<phase-dir>" --roster "<one-line>"  # token 2 (after an explicit "kick it off")
 ...
 *   node signoff.js check --gate spawn|questions [--roster "…"] [--round "…"] [--max-age <sec>]
 *                                                          # exit 0 if a FRESH matching token exists, else 1
 *   node signoff.js status                                 # print both tokens' freshness (human-readable)
 *   node signoff.js clear                                  # remove ALL tokens: questions + every per-round spawn (round done / reset)
 *   node signoff.js --help
 *
 * CONVENTIONS: zero deps (Node built-ins only). `node signoff.js …`. Also a module
