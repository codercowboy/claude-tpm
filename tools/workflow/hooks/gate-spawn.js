#!/usr/bin/env node
/**
 * gate-spawn.js — a Claude Code PreToolUse HOOK that blocks a tpm-workflow subagent
 ...
 *
 * THE GATE:
 *   A marked spawn is allowed ONLY if signoff.js reports a FRESH "spawn" sign-off token (Token 2 —
 ...
 *       "hooks": [ { "type": "command", "command": "node <abs>/tools/workflow/hooks/gate-spawn.js" } ] } ] } }
...
function main() {
  ...
  let signoff;
  try {
    signoff = require(path.join(__dirname, '..', 'signoff.js'));
  } catch (_e) {
  ...
  process.stderr.write(
    ...
    + 'yes, (3) record it: `node tools/workflow/signoff.js spawn --roster "<one-line>"`, then re-spawn. '
