#!/usr/bin/env node
/**
 * task.js — the tpm-task tool: a portable, zero-dependency CLI over a per-task-file store.
 *
 ...
 * (see lib/format.js — the surgical in-place rewrite that makes that hold).
 *
 ...
 *
 * EXAMPLES
 *   node task.js --tasks-dir /tmp/store add --from ./payload.md
 *   node task.js --tasks-dir /tmp/store list --order oldest --state all
 *   node task.js --tasks-dir /tmp/store finish 1245 --action "shipped in PR #42"
 */
...
const { resolveTasksConfig, tasksDirAbs } = require('./lib/config');
...
function cmdResolve(args, ctx) {
  ...
  if (args.since) {
    ...
    sinceOk = (row) => (row.created || '') >= cutoff; // created-based window (ended pool would use ended; see task.md/G6)
  }
...
function main() {
  ...
  if (args.help) { process.stdout.write(`Usage: node task.js [--tasks-dir <p>] ${USAGE[subcommand]}\n`); process.exit(0); }
