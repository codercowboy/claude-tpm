# `lib/config.js` — the `tasks` config-section resolver

Resolves the `tasks` section of a project's `.claude/claude-tpm/config.json` — merged over
built-in defaults — so `task.js` (every subcommand) and the `tpm-task` skill read the SAME 12 keys
...
scoped to just the `tasks` section, mirroring the shape of `tools/session/lib/config.js`.
...
```
node lib/config.js [--config <path>] (--json | --get <dotted.key> | --tasks-dir) [--help]
```

- Exactly one of `--json` / `--get <key>` / `--tasks-dir` must be passed (with `--help`, none of the
  above). **Verified:** calling with none of them prints `lib/config.js: nothing to do — pass one of
...
- `--config <path>` is **optional.** Default:
  ...
  `cwd` for a `CLAUDE.md` marker (via `lib/paths.js`'s `findRoot`).
...
| Key | Default | Meaning |
|---|---|---|
| `enabled` | `true` | Master switch. `false` ⇒ `task.js` (and the skill) short-circuit with a clear message and do NO store ops. |
...
| `timezone` | `local` | Timezone for created/ended stamps + age computation. `local` is exact end-to-end; a named IANA tz is exact on the calendar-day diff, approximate on the sub-24h band (see `task.md`). |
...
| `--tasks-dir` | Shortcut printing the resolved `tasksDir` as a **bare absolute path** (no JSON quoting), resolved relative to `projectRoot` if not already absolute. Convenient for the skill/other scripts (`--tasks-dir "$(node lib/config.js --tasks-dir)"`). |
...
- **Malformed JSON → defaults + warning, exit 0.** A config that is not valid JSON prints
  ...
  resolves the defaults (a following `task.js list` ran normally, exit 0).
...
```
$ node tools/task/lib/config.js --json
{
  "enabled": true,
  "tasksDir": ".claude/claude-tpm/tasks",
  "startId": 1000,
...
$ node tools/task/lib/config.js --get startId
1000

$ node tools/task/lib/config.js --get autoConfirm.finish
false

$ node tools/task/lib/config.js --tasks-dir
/abs/project/root/.claude/claude-tpm/tasks

$ node tools/task/lib/config.js --config /no/such.json --json
config: --config path does not exist: /no/such.json          # exit 1
```

## See also

- [`../task.md`](../task.md) — the tool reference; every subcommand resolves config through here.
