/*
 * legacy-task-format.js — VERBATIM COPY of the retired tool
 * `../claude-tpm/tools/task/tpm-task-format.js` (copy-then-modify per tool-conventions;
 * build-plan §2d/R3), with ONLY the leading `#!/usr/bin/env node` shebang removed so it
 * loads as a plain module. It is the OLD-format READER for tpm-task-migrate.js — the SAME
 * lenient parser that actually WROTE the live task bodies, so the migrator cannot drift
 * from it. Only parseBody / parseIndex / subtaskProgress / STATES are consumed; the
 * surgical mutators are carried unchanged (kept verbatim to avoid drift).
 * DO NOT hand-edit the parse logic — re-copy from the source if that source changes.
 */
/**
 ...
 *   not specifically manage. The sibling tools/session/tpm-session-format.js `renderNote()` rebuilds a
