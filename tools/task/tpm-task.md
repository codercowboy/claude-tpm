...
**preserves unmanaged human prose** (the G1 surgical-in-place-rewrite in `tpm-task-format.js`).
...
```
npx tpm task [--tasks-dir <path>] [--config <path>] <subcommand> [args]
```
...
- **`--config <path>`** — override the config.json path (default
  ...
  `CLAUDE.md` marker). See [`config.md`](./config.md).
...
```
$ npx tpm task --config off.json --tasks-dir ./store list
...
bodies). See [`README.md`](./README.md) for the module split and [`tpm-task-format.js`](./tpm-task-format.js)'s
...
```
$ npx tpm task --tasks-dir ./store import --from ./tmp/tpm-task/braindump.md
...
- **Default out is config-derived (Q4):** `<tasksDir>/<exportDir>/tasks-export-<YYYY-MM-DD>.md`, with
  ...
  the default.)
```
$ npx tpm task --tasks-dir ./store export all
exported 2 task(s) → /abs/store/tmp/tasks-export-2026-08-30.md
$ npx tpm task --tasks-dir ./store export all --state all --out ./backup.md
...
```
$ npx tpm task --tasks-dir ./store list
...
```
$ npx tpm task --tasks-dir ./store edit 1001 --from ./tmp/tpm-task/edit-1001.md
edited #1001
```
...
```
$ npx tpm task --tasks-dir ./store check 1001.a
checked #1001.A (1/3)
$ npx tpm task --tasks-dir ./store check 1001B
checked #1001.B (2/3)
$ npx tpm task --tasks-dir ./store check 1001.A     # already ticked
#1001.A already checked — no change.
$ npx tpm task --tasks-dir ./store check 1001.Z ; echo "exit=$?"
...
```
$ npx tpm task --tasks-dir ./store finish 1000 --action "shipped in PR #42"
#1000 finished.
$ npx tpm task --tasks-dir ./store reopen 1000     # (after it was later removed)
#1000 reopened.
...
```
$ npx tpm task --tasks-dir ./store finish 1000 --action x   # already finished
#1000 is already "finished" — finish is a no-op.
$ npx tpm task --tasks-dir ./store start 1000 ; echo "exit=$?"
task: cannot start #1000: it is "finished". (reopen it first to start again)
exit=1
$ npx tpm task --tasks-dir ./store remove 1000              # legal from finished
...
not parsed here.
```
$ npx tpm task --tasks-dir ./store resolve all
1000 1001 1004
$ npx tpm task --tasks-dir ./store resolve 1000-1004 --state all
1000 1001 1002 1003 1004
$ npx tpm task --tasks-dir ./store resolve 1004-1000 ; echo "exit=$?"
task: descending range not allowed: 1004-1000 (start must be ≤ end)
exit=1
$ npx tpm task --tasks-dir ./store resolve 2000-2005 --state all ; echo "exit=$?"
...
$ npx tpm task --tasks-dir ./store reindex
...
a stored `Created:` value via `list`.** The hour band lives in the age algorithm proper (`tpm-task-render.js`'s
...
See [`config.md`](./config.md) for the full resolver reference. The 12 configurable keys:
...
- [`README.md`](./README.md) — the per-suite readme (module layout, tests, how to run).
- [`config.md`](./config.md) — the `tasks` config-section resolver (12 keys + flags).
- `tpm-task-format.js` header docstring — the canonical body/index token SSOT (the exact managed lines the
