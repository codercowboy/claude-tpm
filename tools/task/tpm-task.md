# `tpm-task.js` — the tpm-task tool (reference)

`tpm-task.js` is the deterministic engine behind the `/tpm-task` skill: a portable, zero-dependency Node
...
**preserves unmanaged human prose** (the G1 surgical-in-place-rewrite in `lib/tpm-task-format.js`).
...
> reproduce. Where the phase is still staged under `out/`, invoke `node out/tools/task/tpm-task.js`; the
> promoted path is `node tools/task/tpm-task.js` (used in the prose below).

## Running it

```
node tools/task/tpm-task.js [--tasks-dir <path>] [--config <path>] <subcommand> [args]
```
...
```
$ node tools/task/tpm-task.js --help
...
```
$ node tools/task/tpm-task.js --config off.json --tasks-dir ./store list
...
bodies). See [`README.md`](./README.md) for the `lib/` split and [`lib/tpm-task-format.js`](./lib/tpm-task-format.js)'s
...
```
$ node tools/task/tpm-task.js --tasks-dir ./store add --from ./tmp/tpm-task/add-rename.md
...
```
$ node tools/task/tpm-task.js --tasks-dir ./store import --from ./tmp/tpm-task/braindump.md
...
- **Default out is config-derived (Q4):** `<tasksDir>/<exportDir>/tasks-export-<YYYY-MM-DD>.md`, with
  ...
  the default.)
```
$ node tools/task/tpm-task.js --tasks-dir ./store export all
exported 2 task(s) → /abs/store/tmp/tasks-export-2026-08-30.md
$ node tools/task/tpm-task.js --tasks-dir ./store export all --state all --out ./backup.md
...
```
$ node tools/task/tpm-task.js --tasks-dir ./store list
...
```
$ node tools/task/tpm-task.js --tasks-dir ./store show 1000
...
```
$ node tools/task/tpm-task.js --tasks-dir ./store edit 1001 --from ./tmp/tpm-task/edit-1001.md
edited #1001
```
...
```
$ node tools/task/tpm-task.js --tasks-dir ./store check 1001.a
checked #1001.A (1/3)
$ node tools/task/tpm-task.js --tasks-dir ./store check 1001B
checked #1001.B (2/3)
$ node tools/task/tpm-task.js --tasks-dir ./store check 1001.A     # already ticked
#1001.A already checked — no change.
$ node tools/task/tpm-task.js --tasks-dir ./store check 1001.Z ; echo "exit=$?"
...
```
$ node tools/task/tpm-task.js --tasks-dir ./store finish 1000 --action "shipped in PR #42"
#1000 finished.
$ node tools/task/tpm-task.js --tasks-dir ./store reopen 1000     # (after it was later removed)
#1000 reopened.
...
```
$ node tools/task/tpm-task.js --tasks-dir ./store finish 1000 --action x   # already finished
#1000 is already "finished" — finish is a no-op.
$ node tools/task/tpm-task.js --tasks-dir ./store start 1000 ; echo "exit=$?"
task: cannot start #1000: it is "finished". (reopen it first to start again)
exit=1
$ node tools/task/tpm-task.js --tasks-dir ./store remove 1000              # legal from finished
...
not parsed here.
```
$ node tools/task/tpm-task.js --tasks-dir ./store resolve all
1000 1001 1004
$ node tools/task/tpm-task.js --tasks-dir ./store resolve 1000-1004 --state all
1000 1001 1002 1003 1004
$ node tools/task/tpm-task.js --tasks-dir ./store resolve 1004-1000 ; echo "exit=$?"
task: descending range not allowed: 1004-1000 (start must be ≤ end)
exit=1
$ node tools/task/tpm-task.js --tasks-dir ./store resolve 2000-2005 --state all ; echo "exit=$?"
...
$ node tools/task/tpm-task.js --tasks-dir ./store reindex
...
a stored `Created:` value via `list`.** The hour band lives in the age algorithm proper (`lib/tpm-task-render.js`'s
...
- `lib/tpm-task-format.js` header docstring — the canonical body/index token SSOT (the exact managed lines the
