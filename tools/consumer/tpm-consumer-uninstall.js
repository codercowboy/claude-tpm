 ...
 *   There is no env/hooks settings.json step to reverse — install.js never wrote one (gated; see
 *   dev/boot-redesign/install-uninstall-design.md).
 *
 * USAGE
 *   node tools/consumer/tpm-consumer-uninstall.js [options]
 *
 *   Options:
 ...
 *
 *   Examples:
 *     node tools/consumer/tpm-consumer-uninstall.js --check
 *     node tools/consumer/tpm-consumer-uninstall.js --quiet
 *
...
async function doStep(opts, step) {
  ...
  if (res.status !== 0) {
    if (opts.force && /already|not found|not installed|no such|unknown/i.test(res.combined)) {
      process.stdout.write('  (already gone — treating as success under --force)\n');
...
async function runUninstall(opts) {
  ...
  // registered rather than removing a marketplace entry declared at a different scope.)
  const r2 = await doStep(opts, {
    ...
    what: `Untrusts/removes the "${MARKETPLACE_NAME}" marketplace declaration for this project.`,
...
function parseArgs(argv) {
  const a = { dir: process.cwd(), quiet: false, force: false, check: false, help: false };
