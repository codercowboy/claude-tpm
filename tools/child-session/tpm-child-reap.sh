#!/usr/bin/env bash
# tpm-child-reap.sh <session-dir> — finalize an interactive child session once it has
...
set -euo pipefail
SESS_DIR="${1:-}"
[ -n "$SESS_DIR" ] && [ -f "$SESS_DIR/session.json" ] || { echo "usage: tpm-child-reap.sh <session-dir-with-session.json>" >&2; exit 2; }
