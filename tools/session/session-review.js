#!/usr/bin/env node
/**
 * session-review.js — the session-notes READ API (task B2).
 *
 * PURPOSE
 ...
 *   (session-notes-design.md §"The recurring query"). Shares lib/format.js's token SSOT with
 *   session-notes.js (write side) so read/write cannot drift.
 *
 ...
 *
 * EXAMPLES
 *   node session-review.js --sessions-dir claude-context/sessions --last 5
 *   node session-review.js --sessions-dir claude-context/sessions --last 10 --open-items
 *   node session-review.js --sessions-dir claude-context/sessions --last 20 --grep "sessionId"
 *   node session-review.js --sessions-dir claude-context/sessions --last 5 --json
 */
...
const { parseNote } = require('./lib/format');
...
function printHelp() {
  process.stdout.write(
    [
      'Usage: node session-review.js --sessions-dir <dir> --last N [--open-items|--decisions] [--since <date>] [--grep <term>] [--json] [--help]',
      '',
...
function main() {
  ...
  if (!args.sessionsDir || !args.last || Number.isNaN(args.last)) {
    process.stderr.write('session-review.js: --sessions-dir and --last <N> are required.\n\n');
    printHelp();
    process.exit(1);
  }
