#!/usr/bin/env node
/**
 * lib/config.js — the `session` config-section resolver (task A1).
 *
 * PURPOSE
 ...
 *   SAME ~6 keys the SAME way. Mirrors the shape of tools/workflow/config-resolver.js but
 *   scoped to `session` only — per tool-conventions.md Part I §2 (portability), this does
 *   NOT `require()` config-resolver.js; it is a small, deliberately duplicated copy of just
 ...
 *   node lib/config.js --get notes.sessionsDir
...
function printHelp() {
  process.stdout.write(
    [
      'Usage: node lib/config.js [--config <path>] (--json | --get <dotted.key> | --sessions-dir) [--help]',
      '',
      ...
      'Examples:',
      '  node lib/config.js --json',
      '  node lib/config.js --get notes.enabled',
      '  node lib/config.js --sessions-dir',
      '',
    ].join('\n'),
  );
...
function main() {
  ...
  if (!args.json && !args.get && !args.sessionsDir) {
    process.stderr.write('lib/config.js: nothing to do — pass one of --json / --get / --sessions-dir.\n\n');
    printHelp();
    process.exit(1);
  }
