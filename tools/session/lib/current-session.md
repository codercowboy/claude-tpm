# `lib/current-session.js` — current-session state resolver
...
`open`) and `session-notes.js`'s "which folder is CURRENT" guard — both the skill and the write
API call this directly, on the same footing as `session-notes.js`/`session-review.js`.
...
pointer itself is the reliable disambiguator — see the header docstring in `current-session.js`
for the full rationale.

## Requirements / invocation shape

```
node lib/current-session.js --sessions-dir <dir> (--state | --open | --seal | --next-number) [--help]
```

- `--sessions-dir <dir>` — **required.** No default — resolve it via `node lib/config.js
  --sessions-dir` (see `lib/config.md`).
...
```
$ node tools/session/lib/current-session.js --sessions-dir "$SDIR" --state
...
$ node tools/session/lib/current-session.js --sessions-dir "$SDIR" --open   # idempotent
{ "number": "001", "sessionId": "...", "isNew": false, "pointerPath": "..." }

$ node tools/session/lib/current-session.js --sessions-dir "$SDIR" --next-number
001

$ node tools/session/lib/current-session.js --sessions-dir "$SDIR" --seal
{ "number": "001", "closedAt": "2026-08-30T09:16:42.622Z" }

$ node tools/session/lib/current-session.js --sessions-dir "$SDIR" --seal   # nothing open now
...
- `tools/session/lib/config.md` — resolves `--sessions-dir` for this tool (via `--get
  notes.sessionsDir` / `--sessions-dir`).
- `tools/session/session-notes.md` — the write API; calls `resolveCurrentSession`/`sealSession`
