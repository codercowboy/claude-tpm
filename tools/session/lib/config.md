# `lib/config.js` — the `session` config-section resolver
...
portability): it does NOT `require()` `tools/workflow/config-resolver.js`; it is a small,
...
```
node lib/config.js [--config <path>] (--json | --get <dotted.key> | --sessions-dir) [--help]
```
...
| `--sessions-dir` | Shortcut for `--get notes.sessionsDir`, but prints a **bare absolute path** (no JSON quoting) — resolved relative to `projectRoot` if the configured value isn't already absolute. Convenient for other scripts/shells to consume directly (e.g. `--sessions-dir "$(node lib/config.js --sessions-dir)"`). |
...
```
$ node tools/session/lib/config.js --config nonexistent.json --json
config: --config path does not exist: /abs/path/nonexistent.json

$ node tools/session/lib/config.js --json      # no config file at the default location
{
  "enabled": true,
  "notes": { "enabled": true, "sessionsDir": "claude-context/sessions" },
...
$ node tools/session/lib/config.js --sessions-dir
/abs/project/root/claude-context/sessions

$ node tools/session/lib/config.js --get notes.enabled
false                                                    # (with the nested-override config above)

$ node tools/session/lib/config.js --get nope.nope
config: no such key "nope.nope" in resolved config.
```

## See also

- `tools/session/lib/current-session.md` — consumes `--sessions-dir` from this resolver.
- `tools/session/session-notes.md` — the write API; `--sessions-dir` is required on every
