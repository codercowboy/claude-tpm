# `session-review.js` — session-notes READ API
...
Shares `lib/format.js`'s token parser with `session-notes.js` (write side), so read and write
cannot drift apart.
...
```
node session-review.js --sessions-dir <dir> --last N [--open-items|--decisions] [--since <date>] [--grep <term>] [--json] [--help]
```

- `--sessions-dir <dir>` and `--last <N>` are **both required** for any real output. Missing either
  (or a non-numeric `--last`) prints `session-review.js: --sessions-dir and --last <N> are
...
```
$ node tools/session/session-review.js --sessions-dir "$SDIR" --last 3
...
session-001 — 2026-08-30 — Documentarian sandbox verification [SEALED 2026-08-30]
  Where: Verifying session-notes.js
  Next:  write session-review.js checks
session-000 — (legacy format — not token-parseable) — .../session-000/notes.md

$ node tools/session/session-review.js --sessions-dir "$SDIR" --last 5 --open-items
...
$ node tools/session/session-review.js --sessions-dir "$SDIR" --last 5 --decisions
session-001 — Decided: single pointer, not a sessionId map — env var unreliable outside subagents

$ node tools/session/session-review.js --sessions-dir "$SDIR" --last 5 --since 2026-08-31
session-002 — 2026-08-31 — second sandbox session [SEALED 2026-08-30]
  Where: second session state
  Next:  none pending
session-000 — (legacy format — not token-parseable) — .../session-000/notes.md
...
- `tools/session/session-notes.md` — the write side (same token format, same `--sessions-dir` flag).
- `tools/session/lib/format.js` — the canonical format's token/regex source of truth this tool parses with.
- `out/skills/tpm-session/SKILL.md` — the skill's `info` mode is a thin wrapper over
  `current-session.js --state` plus a session count, not this tool directly; `save`/`open`/`close`
  call `session-notes.js`, not `session-review.js` — this tool is for the human/orchestrator
