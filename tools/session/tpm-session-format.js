#!/usr/bin/env node
/**
 * tpm-session-format.js — the session-notes format token SSOT (task B1's shared dependency).
 *
 * PURPOSE
 ...
 *   landmarks from it — don't hardcode a second copy"). `tpm-session-notes.js` (write) and
 *   `tpm-session-review.js` (read) both import this — so write/read tokens cannot drift.
 *
...
// Matches ANY bracketed tag, not just STATUS_TAGS — tpm-session-notes.js's own docstring
 ...
 * tpm-session-notes.js after it mutates the parsed structure).
 */
