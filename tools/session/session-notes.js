#!/usr/bin/env node
/**
 * session-notes.js — the session-notes WRITE API (task B1).
 *
 * PURPOSE
 ...
 *   `session-notes.md` (resolved via lib/current-session.js — never a stale/other session),
 ...
 *       closed (lib/current-session.js sealSession) so the next bare `tpm-session` invocation
 *       opens fresh rather than reusing this session.
 *
 * IMMUTABILITY GUARD
 *   Every verb above operates ONLY on the CURRENT open session (per
 *   lib/current-session.js#resolveCurrentSession). If no session is open, every verb except
 ...
 *   `lib/current-session.js#openSession`'s job, invoked by the `tpm-session open` skill mode.
 ...
 *
 * CLI
 *   --sessions-dir <dir>   REQUIRED on every invocation (resolve via lib/config.js --sessions-dir).
 *   --edit-sealed <NNN> --confirm   Override the current-session-only guard (see above).
 *   --help
 *
 * EXAMPLES
 *   node session-notes.js --sessions-dir claude-context/sessions resume \
 *     --where "Built the tools/session suite" --next "write tests" --in-flight "none"
 *   node session-notes.js --sessions-dir claude-context/sessions open add --owner jason "review the config-guide patch"
 *   node session-notes.js --sessions-dir claude-context/sessions open done 1
 *   node session-notes.js --sessions-dir claude-context/sessions log --status WIP "wrote lib/format.js"
 *   node session-notes.js --sessions-dir claude-context/sessions decide "single pointer, not a sessionId map" --why "env var unreliable outside subagents"
 *   node session-notes.js --sessions-dir claude-context/sessions seal
 */
...
const { resolveCurrentSession, sealSession } = require('./lib/current-session');
const { parseNote, renderNote, nextOpenItemId, buildTitle } = require('./lib/format');
...
function printHelp() {
  process.stdout.write(
    [
      'Usage: node session-notes.js --sessions-dir <dir> [--edit-sealed <NNN> --confirm] <verb> [args]',
      '',
      'Verbs:',
      '  init [--theme "<text>"] [--date <YYYY-MM-DD>]',
      '  resume --where "<text>" --next "<text>" [--in-flight "<text>"]',
...
function main() {
  ...
  if (!args.sessionsDir) {
    process.stderr.write('session-notes.js: --sessions-dir is required.\n\n');
    printHelp();
    process.exit(1);
  }
  ...
  const verb = args._[0];
  if (!verb) {
    process.stderr.write('session-notes.js: a verb is required (init|resume|open|log|decide|seal).\n\n');
    printHelp();
    process.exit(1);
  }
  ...
  try {
    ...
    switch (verb) {
      ...
      default:
        process.stderr.write(`session-notes.js: unknown verb "${verb}".\n\n`);
        printHelp();
        process.exit(1);
    }
    process.exit(0);
